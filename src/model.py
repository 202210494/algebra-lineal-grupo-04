...
class ProyeccionModel:
    ...
    def calc_con_rotacion(self, P_P, PL_M, rotacion):
        ...
        self.puntos["P"] = Punto(self.rotar_punto(P_P, rotacion))
        self.puntos["P_Rotar"] = Punto(P_P)

        # calculamos la proyeccion desde el nuevo punto 
        self.calcular_proyeccion(self.puntos["P"].to_numpy(), PL_M)
        ...
        desc += (f"Punto ha rotar: {self.puntos["P_Rotar"].parsed_point()}\n")
        desc += (f"Punto proyectado rotado: \n{self.puntos["P_"].parsed_point()}\n\n")
    ...
    def calc_max_v(self):
        m_s = 0
        for e in self.vectores.values():
            v = e.to_numpy()
            c_s = np.sqrt(v[0]**2 + v[1]**2 + v[2]**2) # get modulo
            if(c_s > m_s):
                m_s = c_s
